...
$file = fopen('Validation.csv', 'r');

while (($line = fgetcsv($file,';')) !== FALSE) {
  // $line est un tableau des éléments du CSV
  $name = $line[0]; // Nom du produit
  $image = $line[1];
  $motorization = $line[2];
  $price = $line[3];
  $difficulty = $line[4];
  ...
  echo "<a href='page_recette_Admin_Verif.php?name=" . urlencode($name) . "&price=" . urlencode($price) . "&notice=" . urlencode($notice) . "' style='text-decoration: none;'><h2>$name</h2></a>";
  echo "</div>";
}
